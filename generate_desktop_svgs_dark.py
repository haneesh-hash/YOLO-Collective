...
# Square format template for WhatsApp and Web Profiles (1:1 ratio) with dark background
...
for brand in brands:
    # Standard format
    standard_content = svg_template_standard_dark.format(
    ...
    # Square format
    square_content = svg_template_square_dark.format(
        brand_name=brand['name'], 
        font_size=brand['font_size'], 
        letter_spacing=brand['spacing']
    )
...
print(f"Generated {len(brands) * 2} Dark SVG files to {desktop_dir}")
